...
1. **Fetch the request.** Run `node scripts/inbox-fetch.mjs "$REQUEST_ID"` from `schemanager/`. It returns `{id, text, status, tier, proposal, ...}`. Verify `status === 'executing'` — otherwise abort with a note.
...
4. **Verify.** Run `cd schemanager && npm run build` — must pass. If a lint script exists (`npm run lint`), run it too. On failure, mark `failed` with the error text and stop.
